...
meta_html_jobs_dir = currentdir + '/jobs/html'
meta_json_jobs_dir = currentdir + '/jobs/json'
...
def parse():
    ...
    for html_file_name in os.listdir(meta_html_jobs_dir):
        ...
        if file_name in visited:
            ...
            continue
        meta_job_html_file_path = os.path.join(meta_html_jobs_dir, html_file_name)
        page_str=''
        try:
            with open(meta_job_html_file_path, 'r') as file:
                page_str = file.read()
            ...
            if reply:
                ...
                job_json['url'] = meta_job_url.format(file_name)

                json_path = os.path.join(meta_json_jobs_dir, file_name + '.json')
                with open(json_path, 'w') as file:
                    file.write(json.dumps(job_json))
                visited.add(file_name)
                logging.info('written: ' + json_path)
            else:
                failed_path.append(meta_job_html_file_path)                
        except:
            failed_path.append(meta_job_html_file_path)
            traceback.print_exc()
